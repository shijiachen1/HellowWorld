...
def create_ctrler(args, weights_init):
    # controller = eval('models_search.'+args.controller+'.Controller')(args=args).cuda()
    # controller.apply(weights_init)
    # ctrl_cell0_optimizer = torch.optim.Adam([{'params':controller.cell0_encoder.parameters()}, {'params':controller.cell0_decoder.parameters()}],
    #                                  args.ctrl_lr, (args.beta1, args.beta2))
    # ctrl_cell1_optimizer = torch.optim.Adam([{'params':controller.cell1_encoder.parameters()}, {'params':controller.cell1_decoder.parameters()}],
    #                                  args.ctrl_lr, (args.beta1, args.beta2))
    # ctrl_cell2_optimizer = torch.optim.Adam([{'params':controller.cell2_encoder.parameters()}, {'params':controller.cell2_decoder.parameters()}],
    #                                         args.ctrl_lr, (args.beta1, args.beta2))
    # return controller, ctrl_cell0_optimizer, ctrl_cell1_optimizer, ctrl_cell2_optimizer
    controller = eval('models_search.' + args.controller + '.Controller')(args=args).cuda()
    controller.apply(weights_init)
...
def main():
    ...
    if args.load_path:
        ...
        #controller, ctrl_cell0_optimizer, ctrl_cell1_optimizer, ctrl_cell2_optimizer = create_ctrler(args, weights_init)
        controller, ctrl_optimizer = create_ctrler(args, weights_init)
        ...
        #ctrl_cell1_optimizer.load_state_dict(checkpoint['ctrl_cell1_optimizer'])
        #ctrl_cell2_optimizer.load_state_dict(checkpoint['ctrl_cell2_optimizer'])
    ...
    for search_iter in tqdm(range(int(start_search_iter), int(args.max_search_iter)), desc='search progress'):
        ...
        #     prev_archs, prev_hiddens = get_topk_arch_hidden(args, controller, gen_net, dis_net, prev_archs, prev_hiddens)
        #
        #     # grow section
        #     del controller
        #     del ctrl_optimizer
        #     controller, ctrl_arch_optimizer, ctrl_optimizer = create_ctrler(args, cur_stage, weights_init)
        #
        ...
        dynamic_reset = None
        # train_controller(args, controller, ctrl_optimizer, gen_net, dis_net, eval_loader, writer_dict)
        ...
        save_checkpoint({
            ...
            #'ctrl_cell1_optimizer': ctrl_cell1_optimizer.state_dict(),
            #'ctrl_cell2_optimizer': ctrl_cell2_optimizer.state_dict(),
            'prev_archs': prev_archs,
            'prev_hiddens': prev_hiddens,
            'path_helper': args.path_helper
        }, False, args.path_helper['ckpt_path'])

    final_archs, _ = get_topk_arch_hidden(args, controller, gen_net, dis_net)
