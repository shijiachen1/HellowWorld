...
class Generator(nn.Module):
    ...
    def set_arch(self, arch_id, total_cells):
        if not isinstance(arch_id, list):
            arch_id = arch_id.to('cpu').numpy().tolist()
        arch_id = [int(x) for x in arch_id]
        self.total_stage = total_cells
        for cur_stage in range (self.total_stage):
            if cur_stage == 0:
    ...
    def forward(self, z, hiddens=None):
        if hiddens:
            z = z + torch.sum(hiddens[0],dim=0)
        ...
        for cur_stage in range(self.total_stage):
            if cur_stage == 0:
                out.append(self.to_rgb(h1))
            elif cur_stage == 1:
                if hiddens:
                    z = z + torch.sum(hiddens[1],dim=0)
                    h = self.l1(z).view(-1, self.ch, self.bottom_width, self.bottom_width)
                    h1_skip_out, h1 = self.cell1(h)
                h2_skip_out, h2 = self.cell2(h1, (h1_skip_out,))
                out.append(self.to_rgb(h2))
            else:
                if hiddens:
                    z = z + torch.sum(hiddens[2],dim=0)
                    h = self.l1(z).view(-1, self.ch, self.bottom_width, self.bottom_width)
                    h1_skip_out, h1 = self.cell1(h)
                    h2_skip_out, h2 = self.cell2(h1, (h1_skip_out,))
                    #h2 = self.l1(z).view(-1, self.ch, self.bottom_width, self.bottom_width)
                _, h3 = self.cell3(h2, (h1_skip_out, h2_skip_out))
                macro_out = self.to_rgb(h3)
                out.append(macro_out)
        # print("macro_out :{}\n".format(macro_out))
        # print("out :{}".format(out))
        return macro_out, out
...
class Discriminator(nn.Module):
    def __init__(self, args, activation=nn.ReLU()):
        ...
        self.ch = args.df_dim
        self.activation = activation
        #self.l1 = nn.Linear(args.hidden_size*2, args.img_size*3*3)
    ...
    def forward(self, x):
        ...
        # if macro:
        constant_model = nn.Sequential(*layers)
        h = constant_model(h)
        # else:
